...
function checkSearch(){
		...
		if(search()){
			...
			document.ProgramSearch.action="<@spring.url '/popup/RmsProgramSearch.ssc'/>";
...
function save(){
		...
		opener.document.getElementById('pcode').value=ProgramSearch.programCode.value;
            ...
            <tr><th>채널</th><th>프로그램코드</th><th>프로그램명</th><th>방송일</th><th>방송시각</th><th>프로파일 설정</th></tr>
				...
				<#list contentsTbl as content>
		            ...
		            <tr>
			            ...
			            <#else>
			            <td align="left">
			            	&nbsp;<a href="#" onclick="javascript:showDisHide('ly_pop13','1');getLiveInfo('${a?replace('/', '-')}','${content.channelCode!""}','${content.pgmCd!""}','${content.pgmId!""}','${content.pgmGrpCd!""}','${content.pgmNm?replace("'", "")?replace("\"", "")}','${content.audioModeMain?default("Stereo")}');showDisHide('ly_pop13','0');return false;">${content.pgmNm?substring(0,20)+" ..."}</a>
			            </td>
			            </#if>
