...
function checkSearch(){
		...
		if(search()){
			...
			document.ProgramSearch.action="<@spring.url '/popup/MRmsProgramSearch.ssc'/>";
...
function getLiveInfo(brdDd,channelCode,programCode,pgmId,pgmGrpCd,pgmNm,audioModeMain){
	...
	if(audioModeMain == 'Mono') {
		document.ProgramSearch.audioModeMain.value = '01';
	} else if (audioModeMain == 'Stereo') {
		document.ProgramSearch.audioModeMain.value = '02';
	} else {
		document.ProgramSearch.audioModeMain.value = '03';
	}
	 
	 document.ProgramSearch.programCode.value=programCode;
	 document.ProgramSearch.pgmId.value=pgmId;
	 document.ProgramSearch.pgmGrpCd.value=pgmGrpCd;
	 document.ProgramSearch.pgmNm.value=pgmNm;
	 document.ProgramSearch.brdDd.value=brdDd;
	 document.ProgramSearch.channelCode.value=channelCode;
	 //alert(pgmGrpCd);
...
function save(){
	  ...
	  opener.document.getElementById('pgmId').value=document.ProgramSearch.pgmId.value;
	  opener.document.getElementById('pgmGrpCd').value=document.ProgramSearch.pgmGrpCd.value;
	  opener.document.getElementById('pgmCd').value=document.ProgramSearch.programCode.value;
	
	 
	  //opener.document.getElementById('pgmNm').innerHTML=document.ProgramSearch.pgmNm.value;
	  //opener.document.getElementsByName('pgmNm')[0].value=document.ProgramSearch.pgmNm.value;
	  opener.document.getElementById('brdDd').value=document.ProgramSearch.brdDd.value;
	  opener.document.getElementById('channelCode').value=document.ProgramSearch.channelCode.value;
	  opener.document.getElementById('pid').value=document.ProgramSearch.pgmId.value;
	  opener.document.getElementById('pgmNm').value=document.ProgramSearch.pgmNm.value;
	  opener.document.getElementById('pgmNm2').value=document.ProgramSearch.pgmNm.value;
	  opener.getScode();
...
function reset(){
		//document.ProgramSearch.keyword.value="";		
		document.ProgramSearch.startDt.value="";		
		document.ProgramSearch.endDt.value="";	
		document.ProgramSearch.channelCode2.value="0";			
	}
</script>
<html>
<head>
...
<!-- Popup -->		
		<article class="ly_pop12_3" id="ly_pop13">
...
<!-- Table -->                             
        ...
        <table summary="" id="boardview9">
         <colgroup><col width="60px"></col><col width="80px"></col><col width=""></col><col width="80px"></col><col width="70px"></col><col width="352px"></col></colgroup>
            <thead>
            <tr><th colspan="7">
            	<article class="box2">
	                    ...
	                    <dl>
	                        ...
	                        <dd>
	                        	<select name="channelCode2">
		                        	...
		                        	<option value="61" <#if search.channelCode2?default(0)?number==61>selected</#if>>&#160; DMB </option>
	                        ...
	                        <dt>프로그램코드</dt>
	                        <dd><input type="text" name="keyword" value="<#if search.keyword?exists>${search.keyword}</#if>" onKeyDown="keyEnter();" readonly></input></dd>
	                        <dt></dt>
	                    </dl>
...
<!-- list -->
            <tbody>
            <tr><th>채널</th><th>프로그램코드</th><th>프로그램명</th><th>방송일</th><th>방송시각</th><th>프로파일 설정</th></tr>
				<#assign x = 20>  
            	<#assign y = x>
				<#list contentsTbl as content>
		            ...
		            <tr>
			            ...
			            <#assign size=content.pgmNm?length>
			         	<#if size <= 22>
			         	<td align="left">
			            	&nbsp;<a href="#" onclick="javascript:showDisHide('ly_pop13','1');getLiveInfo('${a?replace('/', '-')}','${content.channelCode!""}','${content.pgmCd!""}','${content.pgmId!""}','${content.pgmGrpCd!""}','${content.pgmNm?replace("'", "")?replace("\"", "")}','${content.audioModeMain?default("Stereo")}');showDisHide('ly_pop13','0');return false;">${content.pgmNm!""}</a>
			            </td>
			            <#else>
			            <td align="left">
			            	&nbsp;<a href="#" onclick="javascript:showDisHide('ly_pop13','1');getLiveInfo('${a?replace('/', '-')}','${content.channelCode!""}','${content.pgmCd!""}','${content.pgmId!""}','${content.pgmGrpCd!""}','${content.pgmNm?replace("'", "")?replace("\"", "")}','${content.audioModeMain?default("Stereo")}');showDisHide('ly_pop13','0');return false;">${content.pgmNm?substring(0,22)+" ..."}</a>
			            </td>
			            </#if>
